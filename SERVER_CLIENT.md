...
| `--torch-compile` | Compile the network with `torch.compile` (off by default). Subsequent predictions are faster. The server runs a dummy warmup forward pass at startup to trigger the (slow) one-time compilation up front, so clients never see the first-prediction delay — startup just takes longer. Recommended here: the server is long-lived, so the compile cost is paid once and amortized across all sessions. |
